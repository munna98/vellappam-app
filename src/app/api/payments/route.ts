...
export async function GET(request: Request) {
  try {
    // No longer need orderBy/limit for payment number generation from API
    // but keeping it general for future filtering if needed.
    const { searchParams } = new URL(request.url);
    const orderBy = searchParams.get('orderBy') || 'paymentDate'; // Default order
    const direction = searchParams.get('direction') || 'desc'; // Default direction
    const limit = searchParams.get('limit'); // Still useful for general queries
...
export async function POST(request: Request) {
  try {
    ...
    // paymentNumber is now auto-generated by Prisma's cuid() default, so no generation logic here.
    ...
    const result = await prisma.$transaction(async (prisma) => {
      ...
      const payment = await prisma.payment.create({
        data: {
          ...
          notes,
          // paymentNumber is automatically set by @default(cuid())
        },
      });
