...
export default function CreatePaymentPage() {
  ...
  const [nextPaymentNum, setNextPaymentNum] = useState('Loading...'); // ⭐ Initial state changed to 'Loading...'

  // Fetch customers and next payment number on mount
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setNextPaymentNum('Error loading number'); // Indicate error
      }
    };
    fetchData();
    ...
    try {
      const response = await fetch('/api/payments', {
        ...
        body: JSON.stringify({
          ...
          amount,
          notes,
          // paymentNumber is generated by the API route
        }),
      });
      ...
      router.refresh(); // Revalidate data on payment list page
    } catch (error: any) {
      console.error(error);
  ...
  const handleResetForm = () => {
    ...
    const fetchNextNumOnReset = async () => {
      try {
        setNextPaymentNum('Loading...'); // Reset to loading state
        const paymentNumRes = await fetch('/api/payments/next-number');
        if (paymentNumRes.ok) {
          const { nextNumber } = await paymentNumRes.json();
          setNextPaymentNum(nextNumber);
          ...
          <div className="flex flex-col md:flex-row md:items-end gap-4">
            <div className="flex-1 w-full md:w-auto">
              <div className="space-y-2">
                ...
                <Input
                  id="paymentNumber"
                  value={nextPaymentNum} // ⭐ Display the fetched next number
                  className="w-[120px]"
                  disabled // Keep disabled as it's auto-generated
                />
              </div>
            </div>
            <div className="flex-1 w-full md:w-auto">
